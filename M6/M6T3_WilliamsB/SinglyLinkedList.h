...
class Node {
public:
   std::string nameNsetting;
   double tokensPerSecond;
   SettingsData* setting;
   Node* next;

  // constructor
     Node(std::string nameNsetting, SettingsData* setting = nullptr) : nameNsetting(nameNsetting), setting(setting) {
       next = nullptr;
   }
};
...
public:
   ...
   void PrintList(std::ostream& printStream, const std::string& separator = ", ") {
      ...
      if (node) {
          ...
          printStream << node->nameNsetting;
          node = node->next;
      }
      while (node) {
         printStream << separator << node->nameNsetting;
         node = node->next;
      }
